...
class UpdateBeamPackTest extends TestCaseGraphQL
{
     ...
     * The graphql method.
     */
    protected string $method = 'UpdateBeamPack';
    ...
    public function test_it_can_update_beam(): void
    {
        ...
        $response = $this->graphql(
            $this->method,
            $updates = $this->generateBeamData($this->beam->code, BeamType::MINT_ON_DEMAND)
        );
        ...
        $this->beam->refresh();
        $this->assertEquals(
            $expected = Arr::except($updates, ['packs', 'flags']),
            $this->beam->only(array_keys($expected))
        );

        $this->beam->loadCount('packs');
        $this->assertEquals($this->beam->packs_count, Cache::get(BeamService::key($this->beam->code)));

        $this->beam->load('packs');
        $response = $this->graphql(
            $this->method,
            $updates = $this->generateBeamData(
                $this->beam->code,
                BeamType::MINT_ON_DEMAND,
                1,
                [],
                [],
                $this->beam->packs->first()->id
            )
        );
        $this->assertTrue($response);
        Event::assertDispatched(TokensAdded::class);
    ...
    public function test_it_cannot_use_on_non_beam_pack(): void
    {
        ...
        $response = $this->graphql(
            $this->method,
            $this->generateBeamData($this->beam->code, BeamType::MINT_ON_DEMAND),
            true
        );
    ...
    public function test_it_will_fail_with_token_exist_in_beam(): void
    {
        ...
        $this->seedBeamPack(1, BeamType::TRANSFER_TOKEN);
        $claim = $this->claims->first();
        $claim->forceFill(['token_chain_id' => $this->token->token_chain_id])->save();
        $response = $this->graphql(
            $this->method,
            array_merge(
                $data = $this->generateBeamData($this->beam->code, BeamType::TRANSFER_TOKEN),
    ...
    public function test_it_will_fail_to_create_beam_with_invalid_file_upload(): void
    {
        ...
        $file = UploadedFile::fake()->createWithContent('tokens.txt', '1');
        $response = $this->graphql($this->method, array_merge(
            $this->generateBeamData($this->beam->code),
            ['packs' => [['tokens' => [['tokenIdDataUpload' => $file]]]]]
        ), true);
        $this->assertArraySubset([
    ...
    public function test_it_will_fail_existing_tokens(): void
    {
        ...
        $updates = array_merge(
            $this->generateBeamData($this->beam->code, BeamType::MINT_ON_DEMAND),
            ['packs' => [['tokens' => [['tokenIds' => [$token->token_chain_id]]]]]]
        );
        ...
        $updates = array_merge(
            $updates,
            ['packs' => [['tokens' => [['tokenIds' => [$token->token_chain_id . '..' . $token->token_chain_id]]]]]]
        );
        $response = $this->graphql($this->method, $updates, true);
        $this->assertArraySubset(
            ['packs.0.tokens.0.tokenIds' => ['The packs.0.tokens.0.tokenIds already exist in beam.']],
        ...
        $updates = array_merge(
            $updates,
            ['packs' => [['tokens' => [['tokenIds' => [$token->token_chain_id . '..' . $token->token_chain_id], 'type' => BeamType::MINT_ON_DEMAND->name]]]]]
        );
        $response = $this->graphql($this->method, $updates, true);
        $this->assertArraySubset(
            ['packs.0.tokens.0.tokenIds' => ['The packs.0.tokens.0.tokenIds already exist in beam.']],
        ...
        //     ]]]]],
        // ];
        // $this->assertNotEmpty($code = $this->graphql('CreateBeamPack', $create));

        // $updates = array_merge(
        //     Arr::only($create, ['packs']),
        //     ['code' => $code]
        // );
        // $response = $this->graphql($this->method, $updates, true);
        // $this->assertArraySubset(
    ...
    {
        $data = $this->generateBeamData($this->beam->code);
        $response = $this->graphql(
            $this->method,
    ...
    public function test_it_can_update_beam_flags(): void
    {
        ...
        $this->assertTrue($response);
        $this->beam->refresh();
        $this->assertTrue(BitMask::getBit(BeamFlag::PAUSED->value, $this->beam->flags_mask ?? 0));
    }
    ...
    public function test_it_will_fail_with_invalid_tokens(): void
    {
        ...
        $response = $this->graphql(
            $this->method,
            array_merge($updates, ['packs' => [['tokens' => [['tokenIds' => [Hex::MAX_UINT256]]]]]]),
            true
        );
        $this->assertArraySubset(
            ['packs.0.tokens.0.tokenIds' => ['The packs.0.tokens.0.tokenIds is too large, the maximum value it can be is 340282366920938463463374607431768211455.']],
            $response['error']
        );

        $response = $this->graphql(
            $this->method,
            array_merge($updates, ['packs' => [['tokens' => [['tokenIds' => ['5..1']]]]]]),
            true
        );
        $this->assertEquals(
            'Variable "$packs" got invalid value "5..1" at "packs[0].tokens[0].tokenIds[0]"; Cannot represent following value as integer range: "5..1"',
            $response['error']
        );
    }
    ...
    protected function generateBeamData(
        ...
        $data = [
            ...
            'end' => Carbon::now()->addDays(random_int(1, 1000))->toDateTimeString(),
            'flags' => $flags,
            'packs' => SupportCollection::times($count, fn () => [
                'id' => $packId,
                'tokens' => [[
                    'type' => $type->name,
